...
int main(int argc, char *argv[])
{
	...
	nxyz=ph_init[0];ph_nsolu=ph_init[3]; //Info << "start phreeqc " << endl;
	...
	int ph_nspc = freak.nspc; 
	const std::vector < double > & z = freak.z; //charge of the species
	const std::vector < double > & diff25 = freak.diff25; //diffusion coeff of the species
	std::vector<double> cw0(ph_nspc,0.);
	
	//writes the inital solutions to file
	std::ofstream outFile(cur_dir/"constant/options/solutions");
	std::vector<float> solu_conc(ph_nsolu*ph_nspc,0.);Info << "nsolu nspc "<<ph_nsolu << " "<< ph_nspc <<endl;
	for (i=0;i<ph_nsolu;i++) // solu number
		{ for (j=0;j<ph_nspc;j++) // component number
			{
			float a = freak.spc[j*ph_nsolu+i];
			outFile << a << "\n"; 
			solu_conc[i*ph_nspc+j] = a; 
			} 
		}
	outFile.close();

	//writes the species names
	std::ofstream outFile1(cur_dir/"constant/options/spcNames");
	for (j=0;j<ph_nspc;j++) // spc number
			{ std::string a = freak.spcNames[j]; outFile1 << a << "\n";  } 
	outFile1.close();
	...
	nxyz=ph_data[0];ph_nsolu=ph_data[3]; //!!! nxyz here is inside the ractive part
	//initiate poro
	...
	c_ph.resize(nxyz*ph_nspc,0);
	for (i=0; i<ph_nspc;i++)
		for (int j=0;j<nxyz;j++) 
			{c_ph[i*nxyz+j] = freak.spc[i*nxyz+j];} 

	//build the Cwi fields and attribute them concentrations
	#include "EK/createSpciFields.H"
	int icnt = 0;
	forAll(Cw,i) {
		for (j=0; j<nxyz;j++){
			Cw[i]()[ractive[j]] = freak.spc[icnt];icnt ++;
	...
	while (runTime.run())
    {
		#include "setDeltaT.H"
		runTime++;
		Info << "time = " << runTime.timeName(runTime.value()) <<  "  deltaT = " <<  runTime.deltaTValue() << endl;
		...
		if (activateReaction==1 && tcnt==0)
		{
			...
			//auto start = std::chrono::high_resolution_clock::now();
				...
				freak.setSpc(c_ph);//transfer c_ph to freak : it does not work to send directly Cwi to freak.c
