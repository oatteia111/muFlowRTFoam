...
int main(int argc, char *argv[])
{
	...
	while (runTime.run())
    {
		...
		Info << "time = " << runTime.timeName(runTime.value()) << "  iTwrite " <<  itWrite <<" tw "<<writetimes[itWrite]<<  endl;
		...
		Info << "time = " << runTime.timeName(runTime.value()) << "  deltaT = " <<  runTime.deltaTValue() << " flgWrite "<<flgWrite<<endl;
		runTime++;
		Info << "time = " << runTime.timeName(runTime.value()) << "  deltaT = " <<  runTime.deltaTValue() << " flgWrite "<<flgWrite<<endl;
